 ...
 */ (function ($, window, undefined) {
	...
	var iView = function (el, options) {
			...
			iv.sliderContent.bind('iView:goSlide', function (event, slide) {
				...
				iv.slider.css('background-size', iv.scaleBg.correctSize);
				var corrPosX = (iv.scaleBg.shiftx > 0) ? iv.scaleBg.shiftx + 'px' : '0%';
				var corrPosY = (iv.scaleBg.shifty > 0) ? iv.scaleBg.shifty + 'px' : '0%';
				iv.slider.css('background-position', corrPosX + ' ' + corrPosY);				
	...
	iView.prototype = {
		...
		startSlider: function () {
			...
			if(imgWidth != iv.defs.width){
				...
				//iv.defs.height = imgHeight;
				//iv.sliderContent.trigger('resize');
			}
					iv.scaleBg = {};

                	var cw = iv.slider.width();
                	var ch = iv.slider.height();
                	if(imgWidth > cw || imgHeight > ch){
                		if(imgWidth / cw < imgHeight / ch){
                			iv.scaleBg.y = ch;
                			iv.scaleBg.x = imgWidth * ch / imgHeight;
                			iv.scaleBg.shiftx = (cw - iv.scaleBg.x) / 2;
                			iv.scaleBg.shifty = 0;
                		}
                		if(imgWidth / cw > imgHeight / ch){
                			iv.scaleBg.x = cw;
                			iv.scaleBg.y = imgHeight * cw / imgWidth;
                			iv.scaleBg.shifty = (ch - iv.scaleBg.y) / 2;
                			iv.scaleBg.shiftx = 0;
                		}                		
                	}else{
                		iv.scaleBg.y = imgHeight;
                		iv.scaleBg.x = imgWidth;
                		iv.scaleBg.shiftx = (cw - iv.scaleBg.x) / 2;
						iv.scaleBg.shifty = (ch - iv.scaleBg.y) / 2;
                	}

                	iv.scaleBg.correctSize = iv.scaleBg.x + "px " + iv.scaleBg.y + "px";
		...
		addStrips: function (vertical, opts) {
			...
			for (var i = 0; i < opts.strips; i++) {
				...
				var bgPosition = iv.scaleBg.shiftx - ((stripWidth + (i * stripWidth)) - stripWidth);
					bgPosition = bgPosition + 'px ' + ((iv.scaleBg.shifty > 0) ? (iv.scaleBg.shifty + 'px') : '0%');
				var top = ((vertical) ? (stripHeight * i) + 'px' : '0px'),
					left = ((vertical) ? '0px' : (stripWidth * i) + 'px');

				if (vertical){
					bgPosition = iv.scaleBg.shifty - (i * stripHeight);
					bgPosition =  ((iv.scaleBg.shiftx > 0) ? (iv.scaleBg.shiftx + 'px') : '0%') + ' ' + bgPosition + 'px';
				}
				...
				var strip = $('<div class="iview-strip"></div>').css({
					...
					opacity: 0,
					backgroundSize: iv.scaleBg.correctSize
				});
		...
		addBlocks: function () {
			...
			for (var rows = 0; rows < iv.options.blockRows; rows++) {
				for (var columns = 0; columns < iv.options.blockCols; columns++) {
					...
					var bgPosition = (iv.scaleBg.shiftx - (columns * blockWidth)) + 'px ' + (iv.scaleBg.shifty - (rows * blockHeight)) + 'px';
		...
		goTo: function (action) {
            ...
            try {
                ...
                img_id.onload = function () {
                	...
                	if(this.width > cw || this.height > ch){
                		if((this.width / cw < this.height / ch) || (this.width < cw && this.height > ch)){
                			iv.scaleBg.y = ch;
                			iv.scaleBg.x = this.width * ch / this.height;
                			iv.scaleBg.shiftx = (cw - iv.scaleBg.x) / 2;
                			iv.scaleBg.shifty = 0;
                		}else if((this.width / cw > this.height / ch) || (this.width > cw && this.height < ch)){
                			iv.scaleBg.x = cw;
                			iv.scaleBg.y = this.height * cw / this.width;
                			iv.scaleBg.shifty = (ch - iv.scaleBg.y) / 2;
                			iv.scaleBg.shiftx = 0;
                		}                		
                	}else{
                		iv.scaleBg.y = this.height;
                		iv.scaleBg.x = this.width;
                		iv.scaleBg.shiftx = (cw - iv.scaleBg.x) / 2;
						iv.scaleBg.shifty = (ch - iv.scaleBg.y) / 2;
                	}
			...
			iv.runTransition(fx);
                ...
                img_id.src = iv.defs.image.data('iview:image');
            } catch (e) {
                console.log(e);
            }
			//iv.runTransition(fx);
	...
	$.event.special[str_resize] = {
		...
		add: function (handleObj) {
			...
			function new_handler(e, w, h) {
				...
				data.w = (!!w) ? w : elem.width();
				data.h = (!!h) ? h : elem.height();
